...
namespace CargoWise.eAdapterInboundWebClient
{
	public class eAdapterInboundWebClient
	{
		...
		public static void SendMessage(string serviceAddress, string message, string recipientId, string senderId, string password)
		{
            //string messageNamespace = GetMessageNamespace(messageFilePath);

            string messageNamespace = "http://www.cargowise.com/Schemas/Universal/2011/11";


            using (var fileStream = GetFileAsStream(message))
			{
				using (var adapter = new eHubAdapter(GetConfiguration(serviceAddress), senderId, password))
				{
